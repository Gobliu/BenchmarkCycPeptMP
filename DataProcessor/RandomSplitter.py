...
def random_splitter(df, repeat_split=10):
    ...
    print('length 6, 7, 10 only', len(df))
    ...
    for i in range(repeat_split):
        ...
        # print(len(unique_df), len(train_df), len(temp_df), len(valid_df), len(test_df))
        # Print sizes of train, validation, and test sets
        print("Duplicated data as train:", len(duplicated_df))
    ...
    combined_df = adding_label(combined_df)
