...
def data_loader(ip_df, split_seed, loader):
    ...
    std = np.std(train.y)
    # transformer = dc.trans.NormalizationTransformer(transform_y=False, dataset=train)
    # train = transformer.transform(train)
    # valid = transformer.transform(valid)
    # test = transformer.transform(test)
    print(f'Got {len(train.y)} samples for train, {len(valid.y)} for valid, and {len(test.y)} for test')
    return {'train': train, 'valid': valid, 'test': test, 'mean': mean, 'std': std}


def main(ip_df, op_dir, m_name, task, mode, seed_list):
    ...
    for actual_seed in seed_list:
        ...
        loader = dc.data.CSVLoader(tasks=['Normalized_PAMPA'],
                                   feature_field="SMILES",
                                   id_field="Original_Name_in_Source_Literature",
                                   featurizer=feat)

        data = data_loader(ip_df, loader)
